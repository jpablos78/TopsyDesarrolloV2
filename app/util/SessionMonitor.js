 ...
 * logged out.
 */
Ext.define('Topsy.util.SessionMonitor', {
    singleton: true,
    ...
    window: Ext.create('Ext.window.Window', {
        ...
        modal: true,
        resizable: false,
        title: 'Alerta por Periódo de Inactividad',
        width: 325,
        items: [{
                xtype: 'container',
                frame: true,
                ...
                html: "Su sesión expira automaticamente después de 15 minutos de inactividad. Si su sesión expira, todos los datos que no haya guardado se perderan. </br></br>Si ud. desea continuar trabajando, haga click en el boton 'Continuar Trabajando'.</br></br>"
            }, {
                xtype: 'label',
                text: ''
            }],
        buttons: [{
                ...
                handler: function () {
                    Ext.TaskManager.stop(Topsy.util.SessionMonitor.countDownTask);
                    Topsy.util.SessionMonitor.window.hide();
     ...
     */
    countDown: function () {
        this.window.down('label').update('Su sesión expira en ' + this.remaining + ' segundo' + ((this.remaining == 1) ? '.' : 's.'));
